//! # netpulse-capture
//!
//! The capture-layer pipeline logic: the file-based [`FileCapture`]
...
//! without privileges.
//!
//! The golden rule: capture must never block on analysis. The
...
//! what to drop and records it for honest disclosure.
//!
//! **Phase 1 slice:** the file/import path is fully implemented;
//! live per-OS backends remain in [`netpulse_platform`] as documented stubs.
//!
//! **Phase 5:** the capture layer also owns the lifecycle formats —
...
/// The staged shedding order, preferring to lose
...
/// Honest capture accounting: every drop is counted so a
/// "no drop banner" can be trusted to mean "complete capture". Mirrors the
/// `CaptureStats` the trait exposes for live backends.
...
impl CaptureStats {
    /// Fraction of frames dropped in `0.0.=1.0`.
...
mod tests {
    ...
    fn shedding_is_ordered_detail_before_truth() {
        // Dropping packets is the last resort: ordered greatest.
