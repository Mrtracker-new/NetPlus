...
import { classifyIpAddress } from "../geo/ipClassifier";
import type { BreakdownRow } from "@netpulse/contract";

describe("Zero-Egress Security Invariant Verification", () => {
  it("executes IP classification and GeoIP/ASN enrichment with ZERO network egress", () => {
    clearGeoCaches();

    // Spy / mock all possible browser egress APIs
    ...
    try {
      // Test suite over 1,000 synthetic observed IPs (public, private, loopback, multicast, unmapped)
      const ips = [
        "1.1.1.1",
        "8.8.8.8",
        "9.9.9.9",
      ...
      // Assert zero egress across all channels
